...
- `mix manto.build [--output DIR] [--theme NAME]` — static site generator; renders every page to standalone HTML in `priv/static_site` by default, styled by the active Fabric theme (`--theme` selects a preset or a theme from `manto.json`). Pages keep their folder structure (`docs/guide.md` → `docs/guide.html`), get breadcrumb nav, and every folder gets an auto-generated `index.html`; an explicit `index` page, at the vault root or inside a folder, replaces the index generated for that location. This repo's own site lives in `pages/` (`vault_path` in `manto.json`), and `.github/workflows/pages.yml` publishes the build to GitHub Pages

## Architecture

- `lib/manto/` — core app. `Manto.Content` reads/writes pages from the vault directory (default `priv/content`, overridable via `vault_path` in `manto.json`); `Manto.Content.Parser` wraps MDEx (front matter, GFM tables, and `[[wiki links]]` rewritten to `/editor/Slug` links). `list_pages` returns vault-relative slugs including folders (`docs/guides/setup`); `valid_page_name?/1` allows `folder/name` but rejects `""`, `.`, `..`, and leading `/`
- `lib/manto_web/` — web layer. `MantoWeb.SettingsLive` (`/`) is the settings page where the vault path and site settings (`title`, `description`, `base_url`) are written to `manto.json`; `MantoWeb.EditorLive` (`/editor` and `/editor/*page`) is the split-pane Markdown editor — the wildcard route means `params["page"]` arrives as a list of segments, so `handle_params` joins with `/`, and the sidebar renders a depth-sorted hierarchy via `page_entries/1`
- `lib/mix/tasks/manto.build.ex` — static build. Output keeps folder structure; all links (stylesheet, breadcrumbs, wiki links, tag links) use a `relative_prefix/1` computed as `"../"`×depth so root-relative hrefs resolve from any depth; folder indexes are generated by `write_folder_indexes/2` and the root index by `write_index/4`, both of which defer to an explicit `index` page
...
- **The PWA manifest test fails on `main`** (`pwa_test.exs`: `theme_color` `"#1800ad"` vs expected `"#df4f00"`); pre-existing and unrelated to content/vault work

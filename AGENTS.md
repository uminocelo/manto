...
- `lib/mix/tasks/manto.build.ex` — static build. Output keeps folder structure; all links (stylesheet, breadcrumbs, wiki links, tag links) use a `relative_prefix/1` computed as `"../"`×depth so root-relative hrefs resolve from any depth; folder indexes are generated by `write_folder_indexes/2`
